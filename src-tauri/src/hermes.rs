...
fn is_gateway_ready(state: &AppState) -> bool {
    let child_alive = state.hermes.child.lock().unwrap().is_some();
    let serve_info_ready = state.hermes.serve_info.read().unwrap().is_some();
    child_alive && serve_info_ready
}

#[tauri::command]
pub fn hermes_status(state: State<'_, Arc<AppState>>) -> Value {
    let ready = is_gateway_ready(&state);
    let child_alive = state.hermes.child.lock().unwrap().is_some();

    if !ready && !child_alive && env_gateway_mode() == "serve" {
        // Lazily self-heal on the periodic health check (mirror hermes:status).
        let arc = Arc::clone(&state);
        match spawn_gateway(&arc) {
            Ok(()) => return json!({ "connected": false, "spawning": true }),
            Err(e) => return json!({ "connected": false, "error": e }),
        }
    }

    json!({ "connected": ready })
}
...
fn resolve_hermes_python(hermes_bin: &std::path::Path) -> Option<PathBuf> {
    ...
    let mut _sp = std::process::Command::new(sys_python);

        _sp.arg("--version");

        #[cfg(target_os = "windows")]

        _sp.creation_flags(0x08000000);
...
pub fn hermes_record_start(state: State<'_, Arc<AppState>>) -> Result<String, String> {
    ...
    let child = if which_cmd("arecord").is_some() {
        ...
        let mut _c = std::process::Command::new("arecord");

        _c.args(["-f", "cd", "-t", "wav"]).arg(temp_path.display().to_string())
        ...
        let mut _c = std::process::Command::new("ffmpeg");

        _c.args(["-y", "-f", "pulse", "-i", "default"]).args(["-ac", "2", "-ar", "44100"])
...
fn which_cmd(name: &str) -> Option<std::path::PathBuf> {
    ...
    let mut _wc = std::process::Command::new("which");

    _wc.arg(name).stdin(std::process::Stdio::null()).stdout(std::process::Stdio::piped()).stderr(std::process::Stdio::null());

    #[cfg(target_os = "windows")]

    _wc.creation_flags(0x08000000);

    let out = _wc.output().ok()?;
    ...
    let mut _wv = std::process::Command::new(name);

    _wv.arg("--version").stdout(std::process::Stdio::null()).stderr(std::process::Stdio::null());

    #[cfg(target_os = "windows")]

    _wv.creation_flags(0x08000000);
    ...
    match synthesis_result {
        ...
        Ok(_) => {
            ...
            let player = if let Some(p) = which_cmd("paplay") {
                ...
                let mut _c = std::process::Command::new(p);

                _c.arg(&op).stdout(std::process::Stdio::null()).stderr(std::process::Stdio::null()).stdin(std::process::Stdio::null());
                ...
                _c.args(["-nodisp", "-autoexit", "-loglevel", "quiet"]).arg(&op).stdout(std::process::Stdio::null()).stderr(std::process::Stdio::null()).stdin(std::process::Stdio::null());
                ...
                _c.arg("-q").arg(&op).stdout(std::process::Stdio::null()).stderr(std::process::Stdio::null()).stdin(std::process::Stdio::null());
