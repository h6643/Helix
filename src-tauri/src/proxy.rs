...
//!
//! 设置持久化在 `<helix_data_dir>/proxy.json`（`{"url": "http://…"}`）。
//! 留空 = 直连，不读取系统环境变量。
//!
//! 生效路径（修改后需重启应用）：
//! - 网关子进程：`gateway::build_helix_env` 启动时读代理文件，注入
...
/// 保存代理 URL（空 = 清除）。写 `<helix_data_dir>/proxy.json`。
...
pub fn apply_webview_proxy() {
    #[cfg(target_os = "linux")]
    {
        ...
        if url.is_empty() {
            ...
            let mut settings =
                NetworkProxySettings::new(Some(&url), &["localhost", "127.0.0.1", "[::1]"]);
