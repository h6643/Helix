...
export const createCompactNoticeSlice: StateCreator<CompactNoticeSlice, [], [], CompactNoticeSlice> = (set) => ({
  compressionNotices: {},
  setCompressionNotice: (notice) => set((state) => ({
    compressionNotices: {
      ...state.compressionNotices,
      [notice.sessionId || '__draft__']: notice,
    },
  })),
  clearCompressionNotice: (sessionId) => set((state) => {
  ...
  setBackendCompressionCount: (sessionId, count) =>
    set((state) => ({ backendCompressionCounts: { ...state.backendCompressionCounts, [sessionId]: count } })),
})
