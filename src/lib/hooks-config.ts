...
export type HookType =
  "PreToolUse" | "PostToolUse" | "PreVerify" | "SessionStart" | "SessionEnd";
...
export const HOOK_META: Record<
  HookType,
  { label: string; desc: string; supportsMatcher: boolean }
> = {
  PreToolUse: {
    label: "工具调用前",
    desc: "可拦截/拒绝",
...
export function generateHookId(): string {
  return (
    "hook-" + Date.now().toString(36) + Math.random().toString(36).slice(2, 6)
  );
}
