...
function helixIpc(): any {
  return (typeof window !== 'undefined' && (window as any).electron?.helix) || null
}

async function callMain(method: 'getRawConfig' | 'setRawConfig', patch?: Record<string, any>): Promise<Record<string, any>> {
  const ipc = helixIpc()
  if (!ipc || typeof ipc[method] !== 'function') throw new HelixRestUnavailable()
 ...
 */
export async function patchHelixConfig(patch: Record<string, any>): Promise<void> {
