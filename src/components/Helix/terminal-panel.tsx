...
function stripVerbatimPrefix(p: string | null | undefined): string | null | undefined {
...
function TerminalTabView({ id, isActive }: TerminalTabViewProps) {
  ...
  useEffect(() => {
    ...
    const raf = requestAnimationFrame(() => {
      ...
      electronTerminal.start(id, term.cols, term.rows, cwd).then((res) => {
        if (res?.ok) {
          setTerminalError('')
        } else {
...
export function TerminalPanel({ onClose }: TerminalPanelProps) {
  ...
  const projectName = selectedWorkDir ? selectedWorkDir.split(/[\\/]/).pop() || '终端' : '终端'

  return (
    <div className={`shrink-0 h-64 flex flex-col bg-card border-t border-border/40 rounded-t-lg overflow-hidden ${isTerminalOpen ? '' : 'hidden'}`}>
      ...
      <div className="flex items-center h-8 bg-black/5 backdrop-blur-sm shrink-0 select-none">
        <div className="flex items-center gap-1 px-1 h-full min-w-0 overflow-x-auto scrollbar-hide">
          {tabs.map((tab, i) => {
            ...
            const label = tabs.length > 1 ? `${projectName} ${i + 1}` : projectName
            return (
              <div
                key={tab.id}
              ...
              >
                ...
                <span className="max-w-[140px] truncate text-[calc(var(--helix-transcript-size)*0.8571)]">{label}</span>
                {tabs.length > 1 && (
                  <button
      ...
      <div className="flex-1 min-h-0 flex">
        {tabs.map((tab) => (
          <TerminalTabView key={tab.id} id={tab.id} isActive={tab.id === activeId} />
        ))}
      </div>
    </div>
  )
