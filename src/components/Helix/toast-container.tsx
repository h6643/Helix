...
function ToastIcon({ type }: { type: string }) {
  switch (type) {
    case 'success': return <div className="w-4 h-4 rounded-full bg-emerald-500/20 flex items-center justify-center shrink-0"><Check className="size-2.5 text-emerald-400" /></div>
    case 'error': return <div className="w-4 h-4 rounded-full bg-red-500/20 flex items-center justify-center shrink-0"><XCircle className="size-2.5 text-red-400" /></div>
    case 'warning': return <div className="w-4 h-4 rounded-full bg-amber-500/20 flex items-center justify-center shrink-0"><AlertTriangle className="size-2.5 text-amber-400" /></div>
    default: return <div className="w-4 h-4 rounded-full bg-blue-500/20 flex items-center justify-center shrink-0"><Info className="size-2.5 text-blue-400" /></div>
  }
}
...
export function ToastContainer() {
  ...
  const topToasts = toasts.filter(t => !t.position || t.position === 'top-right')
  ...
  return (
    <>
      ...
      {bottomToasts.length > 0 && (
        <div className="fixed bottom-10 right-4 z-[60] flex flex-col gap-1.5 max-w-xs">
          {bottomToasts.map((toast) => (
            <div
              key={toast.id}
              className={`flex items-center gap-2 bg-card border border-border rounded-xl shadow-lg px-3 py-2 backdrop-blur-sm transition-all duration-200 ${
