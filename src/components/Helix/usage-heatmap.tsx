...
export function UsageHeatmap({ dailyUsage, days = 90, selectedDay, onDaySelect }: UsageHeatmapProps) {
  ...
  useEffect(() => {
    ...
    const recompute = () => {
      ...
      const next = Math.max(MIN_CELL, Math.floor((available + GAP) / weeks) - GAP)
  ...
  const maxVal = Math.max(1, ...cells.map((k) => dailyMap.get(k)?.totalTokens ?? 0))
  const levelOf = (v: number) => (v <= 0 ? 0 : Math.min(LEVELS - 1, 1 + Math.floor(((v - 1) / maxVal) * (LEVELS - 1))))
    ...
    <div ref={containerRef}>
      <div className="overflow-hidden">
        <div style={{ width: gridW, margin: '0 auto' }}>
          {/* 图例：右上角 */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 4, fontSize: 10, color: 'rgba(127,127,127,0.7)', marginBottom: 4 }}>
            <span>较少</span>
            {Array.from({ length: LEVELS }, (_, i) => (
              <div key={i} style={{ width: cell, height: cell, borderRadius: 2, background: levelColor(i) }} />
            ))}
            <span>较多</span>
          </div>
          <div style={{ display: 'flex' }}>
            <div style={{ display: 'grid', gridTemplateRows: `repeat(7, ${cell}px)`, gridTemplateColumns: `repeat(${weeks}, ${cell}px)`, gap: GAP }}>
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        outline: isSel ? '2px solid #3b82f6' : isToday ? '1px solid rgba(127,127,127,0.5)' : 'none',
                        outlineOffset: 1,
                      }}
                    />
                  )
          ...
          <div style={{ display: 'flex', gap: GAP, marginTop: 2 }}>
            {monthLabels.map((m) => (
              <div key={m.col} style={{ width: cell, marginLeft: m.col * (cell + GAP), fontSize: 10, color: 'rgba(127,127,127,0.7)' }}>{m.label}</div>
            ))}
          </div>
        </div>
      </div>
