...
export function RightSidebar() {
  ...
  const [pages, setPages] = useState<PanelPage[]>(() => {
    ...
    if (tab === 'browser') return [{ id: newPageId(), kind: 'browser', url: start }]
    return []
  })
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => { if (e.key === 'Escape') toggleCodeFullscreen() }
  ...
  useEffect(() => {
    ...
    const onDown = (e: MouseEvent) => {
      ...
      if (titlePlusRef.current?.contains(target) || plusMenuRef.current?.contains(target)) return
  ...
  const updatePageTitle = (id: string, title: string) =>
    setPages(prev => prev.map(p => p.id === id ? (p.title === title ? p : { ...p, title }) : p))
  ...
  useEffect(() => {
    if (editorTabs.length === 0 && pages.length > 0 && !pages.some(p => p.id === activePageId)) {
  ...
  useEffect(() => {
    if (pages.length > 0 || editorTabs.length > 0) everHadContentRef.current = true
  ...
  useEffect(() => {
    ...
    if (tab !== null && everHadContentRef.current && pagesRef.current.length === 0 && editorTabsRef.current.length === 0) {
      doRetract()
    }
    ...
    >
      ...
      {(stripPages.length > 0 || editorTabs.length > 0) && (
        <div className="flex items-end gap-0.5 px-1 pt-1 h-9 shrink-0 bg-card border-b border-border/40">
          ...
          >
            {codeFullscreen ? <Minimize2 className="size-3.5" /> : <Maximize2 className="size-3.5" />}
          </button>
        </div>
      )}
      ...
      {plusMenuOpen && typeof window !== 'undefined' && createPortal(
        <div
          ...
          style={{
            top: (titlePlusRef.current?.getBoundingClientRect().bottom ?? 0) + 4,
            right: typeof window !== 'undefined' ? window.innerWidth - (titlePlusRef.current?.getBoundingClientRect().right ?? 0) : 0,
          }}
        >
          <div ref={plusMenuRef}>
            <MoreActionsMenu
