...
import {
  ...
  RefreshCw, RotateCcw, AlertTriangle, Sparkles,
  Minimize2, ChevronLeft, ChevronRight, ChevronDown, Folder, X, Search,
  GitBranch, Keyboard, Keyboard as KeyboardIcon, Anchor, Bell, Volume2,
} from 'lucide-react'
...
const Row = ({ icon, label, description, children }: { icon?: React.ReactNode; label: string; description: string; children: React.ReactNode }) => (
  <div className="grid grid-cols-[14px_1fr_auto] items-center gap-2 py-2.5">
    <span className="text-muted-foreground shrink-0 flex items-center justify-center">{icon}</span>
...
const Group = ({ title, icon, children, indent = true, className = '' }: { title: string; icon: React.ReactNode; children: React.ReactNode; indent?: boolean; className?: string }) => (
  <div className={`pt-5 first:pt-0 ${className}`}>
    <div className="flex items-center gap-2 mb-2 px-0.5 pb-2 border-b border-border/25">
      <span className="text-muted-foreground shrink-0">{icon}</span>
      <h4 className="text-xs font-semibold text-muted-foreground/60 uppercase tracking-[0.08em]">{title}</h4>
    </div>
    <div className={`${indent ? 'pl-[22px]' : ''} divide-y divide-border/25`}>{children}</div>
  </div>
)
        ...
        return (
          <div className="max-w-xl space-y-1">
            ...
            <Group title="预设" icon={<Sparkles className="size-3.5" />}>
              <Row label="当前预设" description="选择不同的 Agent 行为模式">
              ...
              <Row label="自定义指令" description="附加到每个对话的系统提示词">
            ...
            <Group title="数据" icon={<Archive className="size-3.5" />}>
              <div className="py-3 flex flex-wrap gap-2">
                ...
                <Button size="sm" variant="outline" onClick={() => { const i = document.createElement('input'); i.type = 'file'; i.accept = '.json'; i.onchange = async (e) => { const f = (e.target as HTMLInputElement).files?.[0]; if (!f) return; try { const t = await f.text(); const d = JSON.parse(t); if (d.apiConfig) { /* restored */ } await persistToStorage() } catch { showToast({ type: 'error', title: '导入失败' }) } }; i.click() }}>导入配置</Button>
      ...
      case 'appearance':
        return (
          <div className="max-w-xl space-y-1">
            ...
            <Row icon={theme === 'dark' ? <Moon className="size-3.5" /> : <Sun className="size-3.5" />} label="主题" description={theme === 'dark' ? '深色模式' : '浅色模式'}>
              <Toggle2 enabled={theme === 'dark'} onToggle={onToggleTheme} />
            </Row>
            <Group title="编辑器" icon={<FileText className="size-3.5" />} className="mt-3">
              <div className="grid grid-cols-[14px_1fr_auto] items-center gap-2 py-2.5">
                <span />
        ...
        return (
          <div className="max-w-xl space-y-1">
            ...
            <Group title="自动提交" icon={<GitBranch className="size-3.5" />}>
              <Row icon={<GitBranch className="size-3.5" />} label="Agent 完成后自动 commit" description="自动将变更提交到当前分支">
          ...
          {sidebarCollapsed ? (
            ...
            <>
              <div className="px-3 py-3 border-b border-border/20">
                ...
                <div className="flex items-center gap-2 w-full px-2.5 py-1.5 rounded-lg bg-transparent transition-all duration-200 focus-within:bg-muted/25 hover:bg-muted/15">
                  ...
                  <input ref={searchRef} value={search} onChange={e => setSearch(e.target.value)}
                    ...
                    placeholder="搜索设置..." className="flex-1 bg-transparent border-0 ring-0 text-[13px] text-foreground placeholder:text-muted-foreground/30 focus:outline-none focus:ring-0 focus:border-0 min-w-0" />
