...
import { ToggleLeft, ToggleRight, Loader2, X, RefreshCw } from "lucide-react";
...
export function PluginManager({ onClose }: PluginManagerProps) {
  ...
  const loadBackendPlugins = useCallback(async () => {
    ...
    try {
        ...
        new Promise((_, reject) =>
          setTimeout(
            () =>
              reject(
                new Error(
                  "插件列表请求超时（20s），网关可能未实现 plugins.manage",
                ),
              ),
            20_000,
          ),
      ...
      const plugins: BackendPlugin[] = Array.isArray(res?.plugins)
        ? res.plugins
        : [];
  ...
  const handleToggle = async (p: BackendPlugin) => {
    ...
    try {
      const api = helixApi();
      if (!api?.send) throw new Error("Helix 网关不可用");
      const res = await api.send("plugins.manage", {
        action: "toggle",
        name: p.name,
        enable,
      });
      if (res?.plugin) {
        setBackendPlugins((prev) =>
          prev.map((x) => (x.name === res.plugin.name ? res.plugin : x)),
        );
      }
    } catch (e) {
      setBackendError(e instanceof Error ? e.message : String(e));
    ...
    <div className="h-full w-full flex flex-col bg-background">
      ...
      <div className="flex items-center justify-between px-6 py-4 shrink-0">
        <div className="flex items-center gap-2">
          <h2 className="text-[calc(var(--helix-transcript-size)*1.2857)] font-semibold">
            插件管理
          </h2>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-3xl mx-auto px-6 pb-8 space-y-2">
          <div className="flex items-center justify-between">
            ...
            >
              <RefreshCw
                className={`size-3.5 ${backendLoading ? "animate-spin" : ""}`}
              />
            </button>
          </div>
          ...
          {backendLoading && backendPlugins.length === 0 ? (
            ...
            <div className="text-center py-12 text-[length:var(--helix-transcript-size)] text-red-500/80">
              ...
              <p className="text-muted-foreground/50 mt-1 text-[calc(var(--helix-transcript-size)*0.8571)]">
                {backendError}
              </p>
              <button
                onClick={() => loadBackendPlugins()}
            ...
            <div className="text-center py-12 text-[length:var(--helix-transcript-size)] text-muted-foreground/60">
              暂无后端插件，可在 Helix 中执行 helix plugins install owner/repo
              安装
            </div>
          ) : (
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {p.source === "bundled" ? (
                      <span className="text-[calc(var(--helix-transcript-size)*0.7143)] px-1.5 py-0.5 rounded bg-muted text-muted-foreground">
                        内置
                      </span>
                    ) : (
                      <span className="text-[calc(var(--helix-transcript-size)*0.7143)] px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-500 font-medium">
                        用户
                      </span>
                    )}
                    {p.status === "enabled" ? (
                      <span className="text-[calc(var(--helix-transcript-size)*0.7143)] px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-500">
                        已启用
                      </span>
                    ) : (
                      <span className="text-[calc(var(--helix-transcript-size)*0.7143)] px-1.5 py-0.5 rounded bg-muted text-muted-foreground">
