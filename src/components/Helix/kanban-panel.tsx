...
export function KanbanPanel(_props: KanbanPanelProps) {
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center gap-2 px-3 pt-3 pb-2">
          <span className={cn('text-[calc(var(--helix-transcript-size)*0.9286)] font-semibold', colors.chip)}>{headerIcon}</span>
          <span className="text-[calc(var(--helix-transcript-size)*0.9286)] font-semibold text-foreground/80">{headerLabel}</span>
          <span className="ml-auto text-[calc(var(--helix-transcript-size)*0.7857)] text-muted-foreground/60">{tasksInCol.length}</span>
        </div>
        <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-1.5">
          {tasksInCol.length === 0 ? (
            <div className="rounded-lg border border-dashed border-border/50 py-6 text-center text-[calc(var(--helix-transcript-size)*0.7857)] text-muted-foreground/40">
              空
            </div>
          ) : (
            tasksInCol.map(task => (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-1.5">
                  <p className="text-[calc(var(--helix-transcript-size)*0.7857)] leading-snug text-foreground/90 line-clamp-2 break-words flex-1">{task.title}</p>
                  <span className={cn('shrink-0 text-[calc(var(--helix-transcript-size)*0.7143)] font-medium px-1 py-0.5 rounded', STATUS_COLORS[task.status].chip)}>
                    {STATUS_ICONS[task.status]}
                  </span>
                </div>
                {/* 下半部分：元信息 */}
                <div className="flex items-center gap-2 mt-1 text-[calc(var(--helix-transcript-size)*0.6429)] text-muted-foreground/60">
                  {task.assignee && (
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-1.5 px-2 pt-1.5 pb-1">
            <span className={cn('text-[calc(var(--helix-transcript-size)*0.7143)] font-semibold', topColors.chip)}>{STATUS_ICONS[topStatus]}</span>
            <span className="text-[calc(var(--helix-transcript-size)*0.7143)] font-semibold text-foreground/80">{STATUS_LABELS[topStatus]}</span>
            <span className="ml-auto text-[calc(var(--helix-transcript-size)*0.6429)] text-muted-foreground/60">{topTasks.length}</span>
          </div>
          <div className="flex-1 overflow-y-auto px-1.5 pb-1 space-y-0.5">
            {topTasks.length === 0 ? (
              <div className="rounded border border-dashed border-border/50 py-1.5 text-center text-[calc(var(--helix-transcript-size)*0.6429)] text-muted-foreground/40">空</div>
            ) : (
        ...
        >
          ...
          <div className="flex items-center gap-1.5 px-2 pt-1.5 pb-1">
            <span className={cn('text-[calc(var(--helix-transcript-size)*0.7143)] font-semibold', bottomColors.chip)}>{STATUS_ICONS[bottomStatus]}</span>
            <span className="text-[calc(var(--helix-transcript-size)*0.7143)] font-semibold text-foreground/80">{STATUS_LABELS[bottomStatus]}</span>
            <span className="ml-auto text-[calc(var(--helix-transcript-size)*0.6429)] text-muted-foreground/60">{bottomTasks.length}</span>
          </div>
          <div className="flex-1 overflow-y-auto px-1.5 pb-1 space-y-0.5">
  ...
  const renderCompactCard = (task: KanbanTask) => (
    ...
    >
      <p className="text-[calc(var(--helix-transcript-size)*0.7143)] leading-[1.15] text-foreground/80 line-clamp-1 break-words truncate">{task.title}</p>
    </div>
  )
  ...
  return (
    <div className="h-full w-full flex flex-col bg-background relative select-none">
      ...
      <div className="shrink-0 flex items-center gap-3 px-6 py-4 border-b border-border/40">
        <h2 className="text-[calc(var(--helix-transcript-size)*1.2857)] font-semibold text-foreground whitespace-nowrap shrink-0">看板</h2>
        ...
        <div className="flex items-center gap-2 ml-2">
          <div className="relative">
            <select
              ...
              className="h-8 pl-3 pr-8 rounded-lg border border-border/60 bg-muted/40 text-[calc(var(--helix-transcript-size)*0.8571)] text-foreground appearance-none focus:outline-none focus:ring-2 focus:ring-ring"
              data-tip="切换看板"
            >
        ...
        <div className="flex items-center gap-1 ml-auto shrink-0">
          ...
          >
            ...
            <span className="text-[calc(var(--helix-transcript-size)*0.8571)] font-medium">拖到这里删除</span>
          </div>
          <button
            onClick={refresh}
            className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-accent/60 transition-colors"
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 h-8 rounded-lg border border-border/60 text-[calc(var(--helix-transcript-size)*0.8571)] font-medium text-foreground hover:bg-accent/60 transition-colors disabled:opacity-50"
          >
            <Plus className="size-3.5" />
            新建任务
          </button>
      ...
      {/* Columns */}
      <div className="flex-1 min-h-0 flex gap-2 overflow-x-auto py-3 pl-10 pr-3">
        {error ? (
          ...
          <div className="w-[460px] max-h-[80vh] overflow-y-auto rounded-xl border border-border/50 bg-card shadow-2xl p-5">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[calc(var(--helix-transcript-size)*0.7857)] text-muted-foreground mb-1">指派给</label>
                  <select
                ...
                <div>
                  <label className="block text-[calc(var(--helix-transcript-size)*0.7857)] text-muted-foreground mb-1">初始状态</label>
                  <select
              ...
              <div className="flex justify-end gap-2 pt-1">
                <button
                  ...
                  className="px-3 py-1.5 text-[calc(var(--helix-transcript-size)*0.8571)] text-muted-foreground hover:text-foreground rounded-lg hover:bg-accent/60 transition-colors"
                >
                  取消
                </button>
                <button
                  ...
                  disabled={busy === 'create'}
                  className="px-4 py-1.5 text-[calc(var(--helix-transcript-size)*0.8571)] bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors disabled:opacity-50 flex items-center gap-1.5"
                >
          ...
          <div className="absolute inset-y-0 right-0 w-[400px] max-w-[85%] flex flex-col border-l border-border/50 bg-background shadow-2xl">
            ...
            <div className="shrink-0 px-4 py-3 border-b border-border/40 flex items-start gap-2">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  {activeTask && (
                    <span className={cn('shrink-0 text-[calc(var(--helix-transcript-size)*0.7143)] font-medium px-1.5 py-0.5 rounded', STATUS_COLORS[activeTask.status].chip)}>
            ...
            {detailLoading ? (
              ...
              <>
                ...
                <div className="shrink-0 px-4 py-2.5 border-b border-border/40 flex flex-wrap gap-1.5">
                  {activeTransitions.map(to => (
                    <button
                      ...
                      className={cn(
                        'px-2.5 py-1 text-[calc(var(--helix-transcript-size)*0.7857)] font-medium rounded-lg transition-colors disabled:opacity-50',
                  ...
                  <button
                    ...
                    disabled={busy === activeTask.id}
                    className="px-2.5 py-1 text-[calc(var(--helix-transcript-size)*0.7857)] font-medium rounded-lg bg-muted text-muted-foreground hover:bg-accent/60 transition-colors disabled:opacity-50 flex items-center gap-1"
                  >
                ...
                {/* Meta */}
                <div className="shrink-0 px-4 py-3 border-b border-border/40 grid grid-cols-2 gap-x-3 gap-y-2 text-[calc(var(--helix-transcript-size)*0.7857)]">
                    ...
                    {assignOpen && (
                      <div className="absolute right-0 left-0 top-full mt-1 z-40 max-h-52 overflow-y-auto rounded-lg border border-border bg-popover shadow-xl py-1">
                        <button
                          ...
                          className="w-full px-3 py-1.5 text-left text-[calc(var(--helix-transcript-size)*0.8571)] text-muted-foreground hover:bg-accent/60"
                        >
                          未指派
                        </button>
                        {assignees.map(a => (
                          <button
                            ...
                            className={cn(
                              'w-full px-3 py-1.5 text-left text-[calc(var(--helix-transcript-size)*0.8571)] hover:bg-accent/60',
                ...
                <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
                  {activeTask.body ? (
                    <div>
                      <div className="flex items-center gap-1.5 text-[calc(var(--helix-transcript-size)*0.7857)] text-muted-foreground mb-1.5">
                        <FileText className="size-3" />
                        描述
                      </div>
                      <p className="text-[calc(var(--helix-transcript-size)*0.9286)] text-foreground/85 whitespace-pre-wrap break-words">{activeTask.body}</p>
                    </div>
                  ) : null}

                  <div>
                    <div className="flex items-center gap-1.5 text-[calc(var(--helix-transcript-size)*0.7857)] text-muted-foreground mb-2">
                        ...
                        {detail.comments.map((c, i) => (
                          <div key={c.id ?? i} className="rounded-lg border border-border/50 bg-muted/20 px-3 py-2">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="text-[calc(var(--helix-transcript-size)*0.7857)] font-medium text-foreground/80">{c.author || '未知'}</span>
                              {c.created_at != null && (
                                <span className="text-[calc(var(--helix-transcript-size)*0.7143)] text-muted-foreground/50">{timeAgo(c.created_at * 1000)}</span>
                              )}
                            </div>
                            <p className="text-[calc(var(--helix-transcript-size)*0.8571)] text-foreground/80 whitespace-pre-wrap break-words">{c.body}</p>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <p className="text-[calc(var(--helix-transcript-size)*0.7857)] text-muted-foreground/50">暂无评论</p>
                    )}
                    <div className="flex gap-2 mt-2">
                      <input
                        ...
                        placeholder="添加评论…"
                        className="flex-1 px-3 h-8 rounded-lg border border-border/60 bg-muted/30 text-[calc(var(--helix-transcript-size)*0.8571)] focus:outline-none focus:ring-2 focus:ring-ring"
                      />
                      <button
                        onClick={handleComment}
                        disabled={!commentText.trim()}
                        className="px-3 h-8 rounded-lg bg-primary text-primary-foreground text-[calc(var(--helix-transcript-size)*0.8571)] font-medium hover:bg-primary/90 transition-colors disabled:opacity-50"
                      >
                        发送
                      </button>
                    </div>
                      ...
                      {showEvents && (
                        <div className="mt-2 space-y-1">
                          {detail.events.map((ev, i) => (
                            <div key={i} className="flex items-start gap-2 text-[calc(var(--helix-transcript-size)*0.7857)] text-muted-foreground/70">
                              ...
                              <div className="min-w-0">
                                ...
                                {ev.payload && (
                                  <pre className="mt-1 text-[calc(var(--helix-transcript-size)*0.7143)] text-muted-foreground/50 whitespace-pre-wrap break-words">{JSON.stringify(ev.payload, null, 1)}</pre>
                                )}
                              </div>
                            </div>
                          ))}
