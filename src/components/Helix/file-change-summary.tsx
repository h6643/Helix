        ...
        {normalizedDiff.split("\n").map((line, i) => {
          if (line.startsWith("+++") || line.startsWith("---")) {
            return (
              <div
                key={i}
                className="bg-purple-500/10 px-2 py-px text-purple-300/80 whitespace-pre-wrap"
              >
                {line}
              </div>
            );
          ...
          if (line.startsWith("@@")) {
            return (
              <div
                key={i}
                className="bg-sky-500/10 px-2 py-px text-sky-300/80 whitespace-pre-wrap"
              >
                {line}
              </div>
            );
          ...
          if (line.startsWith("+")) {
            return (
              <div
                key={i}
                className="bg-emerald-500/10 border-l-2 border-emerald-500 px-2 py-px text-emerald-300/90 whitespace-pre-wrap"
              >
                {line}
              </div>
            );
          ...
          if (line.startsWith("-")) {
            return (
              <div
                key={i}
                className="bg-red-500/10 border-l-2 border-red-500 px-2 py-px text-red-300/90 whitespace-pre-wrap"
              >
                {line}
              </div>
            );
          ...
          return (
            <div
              key={i}
              className="px-2 py-px text-muted-foreground/70 whitespace-pre-wrap"
            >
              {line}
            </div>
          );
  ...
  return (
    <div>
      ...
      >
        ...
        <span className="flex-1 break-all font-mono text-[length:var(--helix-transcript-size)] text-muted-foreground">
