      ...
      <SettingGroup>
        <div className="py-3">
          <SettingRow
            ...
            hint="Agent 完成一轮操作后自动把变更提交到当前分支。"
          >
            <Toggle
              enabled={gitAutoCommit}
              onToggle={() => setGitAutoCommit(!gitAutoCommit)}
            />
          </SettingRow>
          {gitAutoCommit && (
            <SettingRow
              label="提交信息模板"
              hint="自动提交时使用的提交信息模板，可包含 AI 生成的摘要。"
            >
              {textInput(
                gitCommitTemplate,
                setGitCommitTemplate,
                "如: chore: auto-commit changes",
              )}
            </SettingRow>
          )}
        </div>
        ...
        <div className="py-3">
          ...
          >
            <Toggle
              enabled={gitAutoPush}
              onToggle={() => setGitAutoPush(!gitAutoPush)}
            />
          </SettingRow>
          {gitAutoPush && (
            <>
              <SettingRow
                label="远程仓库 URL"
                hint="推送目标仓库地址；留空使用当前分支配置的 remote。"
              >
                {textInput(
                  gitRemoteUrl,
                  setGitRemoteUrl,
              ...
              <SettingRow
                label="Push 前确认"
                hint="推送前先征求你的确认，避免误推。"
              >
                <Toggle
                  enabled={gitPushConfirm}
                  onToggle={() => setGitPushConfirm(!gitPushConfirm)}
                />
              </SettingRow>
            </>
          )}
        ...
        <div className="py-3">
          <SettingRow
            label="自动创建特性分支"
            hint="在 feature 分支上工作时，自动为每次改动创建新分支。"
          >
            <Toggle
              enabled={gitAutoBranch}
              onToggle={() => setGitAutoBranch(!gitAutoBranch)}
            />
          </SettingRow>
          {gitAutoBranch && (
            <SettingRow
              label="分支命名前缀"
              hint="新建分支的前缀，如 feature/ 或 fix/。"
            >
