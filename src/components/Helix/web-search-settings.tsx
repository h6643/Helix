...
export function WebSearchSettings() {
  ...
  const [availableProviders, setAvailableProviders] = useState<string[] | null>(
    null,
  );
          ...
          try {
            ...
            setActiveProviders(
              data.activeProviders ||
                (data.activeProvider ? [data.activeProvider] : []),
            );
        ...
        if (result?.ok && result.config) {
          ...
          if (Array.isArray((result as any).availableProviders)) {
            setAvailableProviders(
              (result as any).availableProviders as string[],
            );
          }
        }
      } catch (e) {
    ...
    <div className="max-w-3xl space-y-4">
      ...
      {availableProviders !== null && displayProviders.length === 0 ? (
        <p className="text-[calc(var(--helix-transcript-size)*0.8571)] text-muted-foreground/60">
          暂无后端搜索插件，请在 helix 中安装 plugins/web/&lt;name&gt;
          后再来此处配置。
        </p>
      ) : (
        <div className="border border-border/50 rounded-xl overflow-hidden divide-y divide-border/50">
          {displayProviders.map((provider) => {
            ...
            const known = SEARCH_PROVIDERS.some(
              (p) => p.pluginName === provider.pluginName,
            );
            return (
              <div key={provider.id}>
                <div className="flex items-center justify-between p-4 hover:bg-accent/30 transition-colors">
                  <div className="flex-1 min-w-0">
                    <p className="ui-text font-medium text-foreground">
                      {provider.name}
                    </p>
                    <p className="text-[calc(var(--helix-transcript-size)*0.8571)] text-muted-foreground/60 mt-0.5">
