 ...
 */
export function BranchPicker({ workDir, currentBranch, onBranchChange, drop = 'down', className }: BranchPickerProps) {
    ...
    if (dirtyCount > 0) {
      try {
        ...
        if (res.ok && res.output) {
          ...
          setConfirmDialog({ open: true, targetBranch: b, files, committing: false })
          setOpen(false)
          return
        }
      } catch (_) { /* fall through to direct switch */ }
    }
  ...
  const commitAndSwitch = async () => {
    ...
    const commitRes = await electronGit.commit(`chore: 切换前自动提交 (${new Date().toLocaleString('zh-CN')})`)
    if (!commitRes.ok) {
      storeActions.showToast({ type: 'error', title: '提交失败', description: commitRes.error })
      setConfirmDialog(prev => ({ ...prev, committing: false }))
      return
    }
    await doSwitch(confirmDialog.targetBranch)
    setConfirmDialog({ open: false, targetBranch: '', files: [], committing: false })
  }
  ...
  const handleCreate = async (name: string) => {
    ...
    if (res.ok) {
      onBranchChange(name)
      setOpen(false)
      storeActions.showToast({ type: 'success', title: `已创建并检出 ${name}` })
      // Refresh list so the new branch shows up next time.
      electronGit.branchList(workDir)
        .then((r: { ok: boolean; branches?: string[] }) => { if (r.ok && r.branches) setList(r.branches) })
        .catch(() => {})
    } else {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className={`absolute ${popoverPosition} w-64 bg-background/95 backdrop-blur-sm rounded-xl border border-border/30 shadow-lg shadow-black/8 z-50 flex flex-col max-h-80`}>
          {/* Search */}
          ...
          <div className="flex-1 overflow-y-auto py-1 min-h-0">
            ...
            {list.filter(b => !search || b.toLowerCase().includes(search.toLowerCase())).map((b) => (
              <button
                key={b}
                type="button"
              ...
              >
                ...
                {b === currentBranch && (
                  <div className="flex items-center gap-2 shrink-0">
                    {dirtyCount > 0 && (
                      <span className="text-[calc(var(--helix-transcript-size)*0.7857)] text-muted-foreground">未提交：{dirtyCount} 个文件</span>
                    )}
            ...
            {!creating ? (
              ...
              <div className="px-3 py-2 space-y-1.5">
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' && newName.trim()) handleCreate(newName.trim())
              ...
              <div className="flex-1 min-h-0 overflow-y-auto max-h-[280px]">
                {confirmDialog.files.map(f => (
                  ...
                  >
                    ...
                    <span className="truncate flex-1 min-w-0 text-foreground/80">{f.name}</span>
            ...
            <div className="shrink-0 px-5 pb-5 flex items-center justify-end gap-2.5">
              ...
              >
                {confirmDialog.committing && (
                  <svg className="animate-spin size-4" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" className="opacity-25"/><path d="M4 12a8 8 0 018-8" stroke="currentColor" strokeWidth="3" strokeLinecap="round" className="opacity-75"/></svg>
                )}
                提交并切换分支…
              </button>
            </div>
