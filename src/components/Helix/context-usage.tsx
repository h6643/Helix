...
function normalizeCategories(
  ...
  return categories
    ...
    .map(category => ({
      ...
      tokens: Math.max(1, Math.round((category.tokens / estimatedTotal) * used)),
    }))
}
...
function ContextUsagePanel({
  ...
  return (
    <div className="absolute bottom-full right-0 mb-2 w-72 bg-card border border-border/60 rounded-xl shadow-lg p-3 z-50">
      <ContextUsageBar used={used} total={total} categories={normalizedCategories} />
      {/* Legend list */}
      ...
      {toolsetList.length > 0 && (
        <div className="mt-3 pt-2 border-t border-border/40">
          ...
          >
            ...
            <span className="text-foreground/50">{showToolsets ? '收起' : '展开'}</span>
          </button>
          {showToolsets && (
            <div className="mt-2 space-y-1">
              {toolsetList.map(t => (
                <div key={t.toolset} className="flex items-center justify-between gap-2 pl-2">
                  <span className="text-[calc(var(--helix-transcript-size)*0.7857)] text-muted-foreground truncate">{t.toolset}</span>
...
export function ContextUsageIndicator() {
    ...
    try {
      ...
      if (result && typeof result === 'object') {
        ...
        if (data.context_percent >= 80 && !autoCompactCooldownRef.current) {
          ...
          const runInFlight = isChatLoading
            ...
            try {
              ...
              if (result && typeof result === 'object') {
                ...
                if (r.status === 'compressed' && Array.isArray(r.messages)) {
                  ...
                  anchorMessageId = msgs.length > 0 ? msgs[msgs.length - 1].id : undefined
                }
                  ...
                  showToast({ type: 'warning', title: r.lock_held ? '压缩进行中' : '自动压缩已中止', description: r.message || (r.status === 'aborted' ? '后端中止了本次压缩' : '另一个压缩任务正在运行') })
                }
              }
              ...
              showToast({ type: 'error', title: '自动压缩失败', description: String((e as Error)?.message ?? e) })
            } finally {
  ...
  const toolsets: Array<{ toolset: string; tool_count: number; schema_tokens: number }> =
  ...
  return (
    <div className="relative" ref={panelRef}>
      ...
      {open && <ContextUsagePanel used={used} total={total} categories={categories} toolsets={toolsets} onClose={() => setOpen(false)} />}
    </div>
  )
}
