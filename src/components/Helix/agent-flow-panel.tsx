                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <div className="relative" ref={folderDropdownRef}>
                      ...
                      >
                        ...
                        <span className="max-w-[120px] truncate">{selectedWorkDir ? (selectedWorkDir.split(/[\/\\]/).pop() || selectedWorkDir) : '选择项目'}</span>
                        <svg className={`size-2.5 text-muted-foreground transition-transform shrink-0 ${showFolderDropdown ? 'rotate-180' : ''}`} xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m6 9 6 6 6-6"/></svg>
                      </button>
                      {showFolderDropdown && (
                                ...
                                <div className="px-3 py-3 space-y-2 bg-muted/20">
                                  ...
                                  <div className="flex gap-2 justify-end pt-1">
                                    ...
                                    <button type="button" onClick={async () => {
                                      ...
                                      await addExternalService({ name: newServerName.trim() || '', host, port, username, authType: 'key' })
                                      storeActions.showToast({ type: 'success', title: '服务器已添加' })
                                    }} className="px-3 py-1 text-[calc(var(--helix-transcript-size)*0.8571)] bg-primary text-primary-foreground rounded-md hover:bg-primary/90 transition-colors">确定</button>
                                  </div>
                                </div>
                              ) : externalServices.length === 0 ? (
                                <div className="px-3 py-4 text-center">
                                  <p className="text-[calc(var(--helix-transcript-size)*0.8571)] text-muted-foreground/50">暂无远程服务器</p>
                                </div>
                              ) : (
                                  ...
                                  {externalServices.map(svc => {
                                    ...
                                    return (
                                      <button key={svc.id} type="button" onClick={async () => {
                                        ...
                                        if (!isConnected) {
                                          try {
                                            ...
                                            if (sshApi) {
                                              ...
                                              if (!result?.ok) {
                                                storeActions.showToast({ type: 'error', title: 'SSH 连接失败', description: String(result?.error || '') })
                                                return
                                              }
                                        ...
                                        const remotePath = 'ssh://' + (svc.username ?? 'user') + '@' + svc.host + ':' + svc.port
                                        ...
                                        <span className={`size-2 rounded-full shrink-0 ${isConnected ? 'bg-green-500' : 'bg-amber-400'}`} />
              ...
              <div>
                ...
                {showAddServerForm ? (
                  ...
                  <div className="max-h-60 overflow-y-auto py-1">
                    ...
                    <button type="button" onClick={() => setShowAddServerForm(true)} className="w-full flex items-center gap-2 px-3 py-2 text-[calc(var(--helix-transcript-size)*0.8571)] text-muted-foreground/60 hover:text-foreground/80 hover:bg-accent transition-colors border-t border-border/20 mt-1">
                      <span className="size-4 flex items-center justify-center border border-dashed border-current rounded text-xs leading-none">+</span>
                      <span>添加服务器</span>
                    </button>
                  </div>
