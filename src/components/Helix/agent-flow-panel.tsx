...
function mergeAdjacentThinking(
  ...
  for (const block of blocks) {
    ...
    if (block.type === "thinking" && prev && prev.type === "thinking") {
      ...
      const content = curC.includes(prevC)
        ? curC
        : prevC.includes(curC)
          ? prevC
          : `${prevC}\n\n${curC}`;
...
function reconcileBlocksWithContent(
  ...
  if (
    normJoined !== normContent &&
    !normJoined.includes(normContent) &&
    !normContent.includes(normJoined)
  )
    return blocks;
...
function ThinkingTimer({
  ...
  useEffect(() => {
    ...
    const tick = () =>
      setDuration(Math.round((Date.now() - questionStartTs) / 1000));
    tick();
...
const QUICK_COMMANDS: { cmd: string; label: string; prompt: string }[] = [
  {
    ...
    label: "代码审查",
    prompt:
      "请审查当前代码变更，检查安全漏洞、性能问题、代码风格，并给出改进建议。",
  },
  {
    cmd: "/fix",
    label: "修复问题",
    prompt: "请分析并修复当前存在的问题。先定位根因，再给出最小改动方案。",
  },
  {
    cmd: "/test",
    label: "编写测试",
    prompt: "请为当前代码编写单元测试，覆盖主要功能路径和边界情况。",
  },
  {
    cmd: "/doc",
    label: "生成文档",
    prompt: "请为当前代码生成清晰的文档注释，包括函数说明、参数说明和示例。",
  },
  {
    cmd: "/refactor",
    label: "重构优化",
    prompt:
      "请重构当前代码，提高可读性、可维护性，消除重复代码，但不改变功能。",
  },
  {
    cmd: "/explain",
    label: "解释代码",
    prompt: "请详细解释当前代码的工作原理、设计思路和关键实现细节。",
  },
  {
    cmd: "/security",
    label: "安全审查",
    prompt:
      "请对当前代码进行安全审查，检查 OWASP Top 10 漏洞、输入验证、权限控制等。",
  },
  {
    cmd: "/optimize",
    label: "性能优化",
    prompt: "请分析当前代码的性能瓶颈，并给出具体的优化方案。",
  },
  {
    cmd: "/summary",
    label: "代码总结",
    prompt: "请总结当前代码的功能、架构和主要模块，给出一份简洁的概述。",
  },
];
...
function ReasoningEffortControl({
  ...
  onChange: (v: ReasoningEffortLevel) => void;
}) {
...
export function AgentFlowPanel() {
  ...
  useEffect(() => {
    ...
    if (wasRunning && !isRunning && isRunningSession) {
      ...
      if (st.emailNotifyEnabled && st.emailConfigured) {
        const lastText =
          (sharedTextBufferRef.current || "").slice(-800) || "(无文本输出)";
  ...
  const BUILTIN_COMMANDS = useMemo(
    () => [
      ...
      },
      {
        name: "resync",
        aliases: ["sync"],
        description: "从后端重新同步当前会话消息（无需重启应用）",
      ...
      {
        name: "reset",
        description: "重置会话（清空对话+上下文）",
        action: "reset" as const,
      },
      {
        name: "image",
        description: "生成一张图片：/image <prompt>",
        action: "image" as const,
      },
      { name: "mcp", description: "管理 MCP 服务器", action: "mcp" as const },
      {
        name: "model",
        description: "切换到模型选择设置",
  ...
  const slashCmd = input.startsWith("/")
    ? input.slice(1).split(" ")[0].toLowerCase()
    : "";
  const matchedQuickCmds = input.startsWith("/")
    ? QUICK_COMMANDS.filter(
        (c) => !slashCmd || c.cmd.slice(1).startsWith(slashCmd),
      )
    : [];
  ...
  const showSlashMenu =
    input.startsWith("/") &&
    slashMenuOpen &&
    (filteredSkills.length > 0 || matchedQuickCmds.length > 0) &&
    !input.includes(" ");
  ...
  const handleSwitchTab = useCallback(
    (sessionId: string) => {
      ...
      debug("[HelixTrace] switchTab", {
        from: useHelixStore.getState().currentSessionId,
        to: sessionId,
      });
  ...
  const handleRun = useCallback(async () => {
    ...
    if (builtinMatch) {
      const builtin = BUILTIN_COMMANDS.find(
        (c) =>
          c.name === builtinMatch[1].toLowerCase() ||
          c.aliases?.includes(builtinMatch[1].toLowerCase()),
      );
      if (builtin) {
        setInputSynced("");
        resetInputHeight();
        switch (builtin.action) {
          case "compact": {
            ...
            try {
              ...
              if (currentSessionId) {
                sessionMapRef.current.set(currentSessionId, {
                  sid,
                  epoch: useGatewayStore.getState().gatewayEpoch,
                });
                persistSessionMap(sessionMapRef.current);
          ...
          case "image": {
            ...
            if (!imagePrompt) {
              storeActions.showToast({
                type: "warning",
                title: "缺少参数",
                description: "/image <你的提示>",
              });
              break;
            }
            try {
              const r = (await helixApi()!.send("image.generate", {
                prompt: imagePrompt,
                aspect_ratio: "square",
              })) as any;
              if (r?.available === false) {
                storeActions.showToast({
                  type: "warning",
                  title: "图片生成不可用",
                  description: r?.error || "后端没有可用的图片生成能力",
                });
                break;
              }
              ...
              storeActions.showToast({
                type: "success",
                title: "图片已生成",
                description: imagePrompt.slice(0, 40),
              });
            } catch (e) {
              storeActions.showToast({
                type: "error",
    ...
    const pendingBlocksRef = {
      current: [] as Array<{ type: "thinking" | "text"; content: string }>,
    };
          ...
          if (b.type === "thinking") {
            next =
              last?.type === "thinking"
                ? [
                    ...next.slice(0, -1),
                    { type: "thinking", content: b.content },
                  ]
            ...
            if (parsed.type === "tool_call") {
              ...
              if (
                ...
                if (finalBlocks && content) {
                  finalBlocks = reconcileBlocksWithContent(
                    finalBlocks,
                    content,
                  );
                }
                ...
                {matchedQuickCmds.map((qc, j) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <span className="text-[calc(var(--helix-transcript-size)*0.9286)] text-foreground block">
                        {qc.label}
                      </span>
  ...
  return (
    <div className="h-full flex flex-col bg-transparent text-foreground relative">
      ...
      <div
        ref={scrollRef}
        className={`flex-1 min-h-0 overflow-y-auto msg-scroll-viewport ${sessionMessages.length === 0 && !hasSteps ? "hide-scrollbar" : ""}`}
      >
