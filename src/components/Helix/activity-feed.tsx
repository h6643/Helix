...
const push = (e: Omit<ActivityEntry, 'id' | 'at'>) => {
  buffer.unshift({ ...e, id: Math.random().toString(36).slice(2), at: Date.now() })
  ...
  useEffect(() => {
    ...
    const sync = () => {
      ...
      if (st.isAgentRunning !== lastRunning.current) {
        push({ kind: st.isAgentRunning ? 'run-start' : 'run-end', text: st.isAgentRunning ? 'Agent 开始运行' : 'Agent 运行结束' })
    ...
    const toolTimer = setInterval(() => {
      ...
      if (toolCount !== lastToolCount.current) {
        if (toolCount > lastToolCount.current) push({ kind: 'tool', text: `执行了工具调用 (共 ${toolCount})` })
      ...
      <div className="relative w-full max-w-sm h-full bg-card border-l border-border/60 shadow-2xl flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b border-border/60">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-[length:var(--helix-transcript-size)] font-semibold">活动流</h2>
          </div>
