      ...
      <div className="rounded-xl border border-border/50 bg-card/50 shadow-sm overflow-hidden">
        ...
        {entries
          ...
          .map(([model, usage]) => (
            <div
              key={model}
              className="grid grid-cols-4 gap-2 px-4 py-2.5 border-b border-border/50 last:border-0 text-[length:var(--helix-transcript-size)]"
            >
              <span className="font-mono text-foreground truncate">
                {model}
              </span>
              <span className="text-right text-foreground/70 font-mono">
        ...
        <div className="grid grid-cols-4 gap-2 px-4 py-2.5 bg-muted/30 text-[length:var(--helix-transcript-size)] font-medium">
          ...
          <span className="text-right font-mono">
            {formatTokens(entries.reduce((s, [, u]) => s + u.prompt, 0))}
          </span>
          <span className="text-right font-mono">
...
export function UsageDetail() {
  ...
  const [rows, setRows] = useState<
    Array<{ tokens: number; cost: number; model: string; timestamp: number }>
  >([]);
    ...
    for (const step of agentSteps) {
      if (step.type === "usage" && step.content) {
        const m = step.content.match(
          /Tokens:\s*(\d+)\s*total\s*\|\s*Cost:\s*\$([\d.]+)(?:\s*\|\s*Model:\s*(\S+))?/,
        );
        if (m) {
          list.push({
    ...
    <section className="space-y-3">
      ...
      <div className="rounded-xl border border-border/50 bg-card/50 shadow-sm overflow-hidden">
        ...
        {rows.map((r, i) => (
          <div
            key={i}
            className="grid grid-cols-4 gap-2 px-4 py-2 border-b border-border/50 last:border-0 text-[length:var(--helix-transcript-size)]"
          >
...
export function TokenUsagePanel() {
  ...
  if (stats.requestCount === 0) {
    return (
      <section className="rounded-xl border border-border/40 bg-muted/20 px-5 py-12 text-center">
        <p className="text-[length:var(--helix-transcript-size)] text-muted-foreground/70">
          尚未获取到用量数据
        </p>
        <p className="text-[calc(var(--helix-transcript-size)*0.8571)] text-muted-foreground/50 mt-1.5">
          运行一次对话后，这里会自动显示 Token 消耗情况
        </p>
      </section>
    );
  }
  ...
  const activeDay =
    selectedDay && days.some((p) => p.day === selectedDay)
      ? selectedDay
      : today.day;

  const dayEntry = dailyMap.get(activeDay);
  const modelRows = dayEntry?.models
    ? Object.entries(dayEntry.models).sort(
    ...
    <section className="space-y-4">
      ...
      <div className="rounded-xl border border-border/40 bg-card">
        ...
        {modelRows.length > 0 ? (
            ...
            <table className="w-full text-[length:var(--helix-transcript-size)]">
              ...
              <tbody>
                {modelRows.map(([model, m]) => (
                  ...
                  >
                    <td className="px-4 py-2 font-mono text-foreground truncate">
                      {model}
                    </td>
          ...
          <p className="px-4 pb-4 text-[length:var(--helix-transcript-size)] text-muted-foreground/50">
            该日无用量数据
          </p>
        )}
      </div>
