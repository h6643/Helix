...
# Thread-local storage for per-tool-call output streaming callback.
# Set via set_output_callback() before tool invocation; read in _drain().
_output_callback_local = threading.local()


def set_output_callback(callback):
    """Set the output streaming callback for the current thread.

    Called from tool_executor.py before invoking a tool so that _drain()
    can forward chunks to the ACP adapter for real-time streaming.
    Pass ``None`` to clear.
    """
    _output_callback_local.callback = callback


def get_output_callback():
    """Get the output streaming callback for the current thread (or None)."""
...
class BaseEnvironment(ABC):
    ...
    def _wait_for_process(
        ...
        # Per-tool-call real-time output streaming callback (Helix integration).
        # Captured on the calling (worker) thread BEFORE the drain thread starts,
        # because threading.local() is not shared across threads.
        _output_callback = get_output_callback()
