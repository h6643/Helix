...
def _safe_config_get(config: dict, *keys, default=None):
    """Walk nested dict keys without blowing up on missing sections."""
    cur = config
    for k in keys:
...
def _read_config_int(config: dict, path: tuple[str, ...], default=None) -> int | None:
    """Read a config value, returning ``default`` when unset or non-int."""
    raw = _safe_config_get(config, *path)
...
def _read_config_float(config: dict, path: tuple[str, ...], default=None) -> float | None:
    """Read a config value, returning ``default`` when unset or non-float."""
    raw = _safe_config_get(config, *path)
    if raw is None or raw == "":
        return default
    try:
    ...
    def _make_agent(
        ...
        *,
        ...
        kwargs = {
            ...
            # Read the remaining agent behaviour knobs from config.yaml so the
            # ACP path matches the CLI/terminal experience. Without these,
            # ACP-spawned agents get AIAgent defaults (max_tokens=None,
            # temperature=None, max_iterations=90), which causes:
            #   - truncated replies (model cap ≠ configured max_output_tokens)
            #   - more-random output (flash model defaults to t=1.0 vs t=0.7)
            #   - long-task stalls ("很笨" / "突然中断" when iteration budget
            #     runs out at 90 instead of config.yaml's max_turns: 150)
            "max_tokens": _read_config_int(config, ("agent", "max_output_tokens")),
            "max_iterations": _read_config_int(config, ("agent", "max_turns"), default=90),
        }
        ...
        _register_task_cwd(session_id, cwd)
        # Build the ephemeral system prompt the same way the CLI/terminal
        # gateway does (gateway/run.py:_load_ephemeral_system_prompt →
        # _get_system_prompt_for_channel). The agent.system_prompt in
        # config.yaml is the user's personality (e.g. "温柔"); without it
        # the agent is a bare tool-calling shell with no behavioural guidance
        # — "很笨" / "记忆力差" / "答非所问" / "用英文回中文提问".
        # custom_instructions (language prefs, etc.) are prepended so they
        # sit in a high-attention position right at the end of the prompt.
        _agent_cfg = config.get("agent") or {}
        _sp = _agent_cfg.get("system_prompt") or ""
        _ci = _agent_cfg.get("custom_instructions") or ""
        parts: list[str] = []
        if _ci and isinstance(_ci, str) and _ci.strip():
            parts.append(_ci.strip())
        if parts:
