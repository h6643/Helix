...
def _build_child_system_prompt(
    ...
    *,
    ...
    child_depth: int = 1,
    identity_system_prompt: Optional[str] = None,
) -> str:
    ...
    """
    identity_opener = (
        identity_system_prompt.strip()
        if identity_system_prompt and identity_system_prompt.strip()
...
def _build_child_agent(
    ...
    role: str = "leaf",
    # Per-call named identity — if it resolves to a system_prompt in
    # delegation.identities, that prompt replaces the generic "focused
    # subagent" opener of the child system prompt.
    ...
    child_prompt = _build_child_system_prompt(
        ...
        identity_system_prompt=_resolve_identity_system_prompt(identity),
    )
...
def delegate_task(
    ...
    if tasks and isinstance(tasks, list):
        ...
        task_list = [{"goal": goal, "context": context, "role": top_role, "identity": identity}]
    else:
...
def _load_identities() -> List[Dict[str, Any]]:
    """Read ``delegation.identities`` as a list of ``{name, system_prompt}``.

    The Helix frontend persists this as a JSON-on-one-line YAML flow value, so
    the shared YAML loader already parses it into a list. A raw JSON string is
    accepted as a defensive fallback. Entries without a name are dropped.
    """
    raw = _load_config().get("identities")
...
def _resolve_identity_system_prompt(identity: Optional[str]) -> Optional[str]:
    """Return the ``system_prompt`` for a named identity, or None if not found."""
    if not identity:
        return None
    name = str(identity).strip()
    if not name:
        return None
    for ident in _load_identities():
        if ident["name"] == name:
            sp = ident.get("system_prompt", "").strip()
            return sp or None
    return None
...
def _build_identity_param_description() -> str:
    """Compose the 'identity' parameter description, listing configured names."""
    try:
        identities = _load_identities()
    except Exception:
        identities = []
    names = [i["name"] for i in identities if i.get("name")]
    if not names:
        return (
            "Optional named persona for the subagent (overrides the default "
            "'focused subagent' identity). None are configured, so this is "
            "ignored — omit it."
        )
    return (
        "Optional named persona for the subagent (overrides the default "
        "'focused subagent' identity). Available: " + ", ".join(names) + ". "
        "Omit for the default identity."
    )
...
def _build_dynamic_schema_overrides() -> dict:
    ...
    overrides_params["properties"]["identity"]["description"] = _build_identity_param_description()
            ...
            "tasks": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "identity": {
                            "type": "string",
                            "description": "Per-task identity override. See top-level 'identity'.",
                        },
                    },
                    "required": ["goal"],
                },
