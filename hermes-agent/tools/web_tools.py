...
# Guard the imports so a missing plugins/web package degrades gracefully
# instead of taking down the whole module (and the built-in backends with it).
try:
    ...
    _firecrawl_backend_help_suffix = lambda: ""  # type: ignore[assignment]
    _get_firecrawl_client = lambda *a, **k: None  # type: ignore[assignment]
    _get_firecrawl_gateway_url = lambda: ""  # type: ignore[assignment]
    _is_tool_gateway_ready = lambda: False  # type: ignore[assignment]
    check_firecrawl_api_key = lambda: False  # type: ignore[assignment]
...
try:
    ...
    _normalize_tavily_documents = lambda *a, **k: None  # type: ignore[assignment]
    _normalize_tavily_search_results = lambda *a, **k: None  # type: ignore[assignment]
    _tavily_request = lambda *a, **k: None  # type: ignore[assignment]
...
try:
    ...
    _get_async_parallel_client = lambda *a, **k: None  # type: ignore[assignment]
    _get_parallel_client = lambda *a, **k: None  # type: ignore[assignment]

try:
    from plugins.web.exa.provider import _get_exa_client  # noqa: F401
except ImportError:
    _get_exa_client = lambda *a, **k: None  # type: ignore[assignment]
