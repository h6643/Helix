...
def _wait_agent(session: dict, rid: str, timeout: float = 0.0) -> dict | None:
    # First agent build performs plugin discovery + optional lazy dependency
    # installs, which on a cold/slow machine can exceed the old hard 30s cap
    # ("agent initialization timed out" on prompt.submit). Default generously
    # (600s) and allow override via HERMES_AGENT_INIT_TIMEOUT (seconds).
    if timeout <= 0:
        timeout = float(os.environ.get("HERMES_AGENT_INIT_TIMEOUT", "600"))
    ready = session.get("agent_ready")
