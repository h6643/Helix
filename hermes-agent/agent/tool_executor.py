...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    def _run_tool(
        ...
        try:
            ...
            pass
        # Set the output streaming callback so _drain() can forward chunks
        # to the ACP adapter for real-time terminal output streaming.
        _fn_name = function_name
        def _output_delta_callback(chunk: str) -> None:
            if agent.tool_progress_callback and chunk:
                try:
                    agent.tool_progress_callback(
                        "tool.output_delta", _fn_name, chunk, None,
                    )
                except Exception:
                    pass
        try:
            from tools.environments.base import set_output_callback
            set_output_callback(_output_delta_callback)
        except Exception:
            pass
        ...
        try:
            ...
            try:
                ...
                pass
            # Clear the output streaming callback for this worker thread.
            try:
                from tools.environments.base import set_output_callback
